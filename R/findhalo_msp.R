  ...
  dashboardSidebar(width = 300,
                   sidebarMenu(
                     ...
                     fluidRow(
                       ...
                       column(4, checkboxInput("norm_int", "Normalize intensity"))
                     ),
...
server <- function(input, output, session){
  ...
  observeEvent(input$click_file, {
    ...
    output$plot1 <- renderPlotly({
      ...
      if (input$norm_int) {
        df <- result()
        df$intensity <- round(df$intensity/max(df$intensity)*100, 1)
      } else if (input$norm_int & input$check_overlay){
        df <- result()
        df$intensity <- round(df$intensity/max(df$intensity)*100, 1)
        data2$intensity <- round(df$intensity/max(df$intensity)*100, 1)
      
      } else if (input$check_overlay){
        df <- result()
