...
#'
#' @param filepath string. Path to mzML/mzXML file
#' @param featlist data.frame. Variable names should be: name, mz, ms_level (has to be either "ms1, "ms2) 
#' @param numTopIons num number of most intensive MS2 ions to extract
...
plotTopMS2Peaks <- function(filepath, featlist, numTopIons = 10, diff = 0.01, mserr = 0.01, rtWindow = 0.3) {
  ...
  ui <- miniUI::miniPage(
    miniUI::gadgetTitleBar("Click on the peaks ions to select retention time and then click Get MS2"),
    miniUI::miniContentPanel(
      shiny::fillRow(flex = c(NA,1),
                     shiny::fillCol(width = "110px",
                                    shiny::textOutput("rtselect"),
                                    shiny::textOutput("rtWindow"),
                                    shiny::actionButton("sync", "Sync 2<-1")),
    ...
    miniUI::miniButtonBlock(
      shiny::actionButton("getMS", "Get MS2"),
