...
# Plot Hilbert curve for ion mobility HRMS
...
path <- "D:/TEST/Vion/"
file <- "LIUVIONStDmix_setting1_replicates_POS_ESI_20241113_1_A,3_1.mzML"
filename <- paste0(path, file)
chunksize <-  10000

#mzml_file <- arcMS::convert_mzml_to_parquet(filepath)
 
##########manual modify mzml_to_parquet.R#################
...
dt_short <- slice_head(dt, n = 50000) |> mutate(idrows = 1:50000)


gghilbertplot(dt, idcol = bin, color = intensity, add_curve = F)
